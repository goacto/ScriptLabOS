...
| Route | Does |
| --- | --- |
| `/dashboard` | Build Status HUD — XP, streak, script counts, achievements, wake-ups, quick-log widget for bugs/viruses |
| `/library` | IDE-style `.gss` editor |
| `/packages` | Bundle scripts into named packages |
| `/day` | Drag `.gss` or 📦 onto an hourly timeline → 3-stage build workflow (Draft → In-Progress → Complete) |
| `/tester` | Pomodoro runner for a selected script (deep-link via `?id=`). Bugs/viruses filtered out. |
| `/profile` | Edit name, wake-ups, values, goals after onboarding |
| `/analytics` | 30-day performance dashboard with charts, completion rates, and build quality metrics |
| `/history` | Git-style commit log of all life events (runs, achievements, builds) with filtering and search |
| `/settings` | OS configuration — theme toggle, data management, tutorials, documentation viewer, about section |
...
### 2.3a Quick-log system for bugs/viruses

Bugs and viruses use a different tracking model than positive habits. They're designed for **minimal friction logging** when you catch yourself running a negative pattern:

- **QuickLogWidget** (`components/QuickLogWidget.tsx`) — renders on `/dashboard` only if bugs/viruses exist. Shows active scripts with "Quick Log" buttons.
- **Quick logs** — record a 0-duration run with `completed: true`. No reflection required, no XP awarded.
- **21-day elimination** — calculates days since last run. Scripts that haven't been logged for 21+ days move to "Eliminated" section.
- **Resurrection** — logging an eliminated script moves it back to active.
- **Filtered from planning** — bugs/viruses don't appear in Day Builder or Tester dropdowns. You can't schedule a bad habit; you can only log when it occurs.
...
### 3.4a Day Builder 3-stage workflow

The Day Builder now uses a **build lifecycle model**:

1. **Draft** — planning mode. Drag scripts/packages onto the timeline. Shows "▶ Start Day" button (disabled if no scripts planned).
2. **In-Progress** — execution mode. Timeline locked (no editing). Shows real-time **execution fidelity** tracking: `(completedScriptsInPlan / plannedScripts) × 100`. Button changes to "✓ Complete Day".
3. **Complete** — shipped. Shows completion timestamp and final execution fidelity score. End-of-day reflection required before transitioning.

**Execution fidelity** compares planned script IDs against actually-completed scripts from today's runs. Color-coded: green (≥80%), amber (≥50%), red (<50%).

**End-of-day reflection** — modal with three mandatory fields (wins, challenges, tomorrow's focus). Stored in `Executable.reflection`. This closes the feedback loop and creates the commit log entry.
...
### 3.4c Responsive navigation

`components/NavBar.tsx` adapts to screen size:

- **Desktop (≥1024px):** Full horizontal nav with all links visible.
- **Mobile (<1024px):** Logo + ⌘K hint + stats + hamburger button (☰). Tapping the hamburger reveals a full-screen dropdown with all navigation links, backdrop overlay, and auto-close on navigation.

The hamburger state is local (`useState`). The overlay uses `z-10` for backdrop, `z-20` for menu, and the header itself is `z-30`. When adding modals, ensure they use `z-50` to float above navigation.

### 3.4d Command palette
...
5. **Decide tracking model:** Should it use Pomodoro sessions (like baseline/update/upgrade) or quick-log (like bug/virus)? If quick-log, filter from Day Builder (`DayBuilder.tsx` line ~370) and Tester (`PomodoroTimer.tsx` line ~21, ~214), and add to QuickLogWidget.
6. Update `project.md` and this guide.

**Current tracking models:**
- **Pomodoro model:** baseline, update, upgrade — full sessions with reflection, scheduled in Day Builder
- **Quick-log model:** bug, virus — 0-duration logs from dashboard widget, 21-day elimination, filtered from planning
...
**Quick-log elimination threshold:** 21 days clean
**Build workflow stages:** Draft → In-Progress → Complete
**Execution fidelity formula:** `(completedPlannedScripts / totalPlannedScripts) × 100`
**Z-index layers:** backdrop (10) · menu (20) · header (30) · modals (50)
**Responsive breakpoint (hamburger):** 1024px (Tailwind `lg`)
**Routes with back buttons:** `/settings`, `/analytics`, `/history`
